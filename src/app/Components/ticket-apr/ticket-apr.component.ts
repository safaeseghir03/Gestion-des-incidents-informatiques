...
@Component({
  selector: 'app-ticket-apr',
  templateUrl: './ticket-apr.component.html',
  styleUrls: ['./ticket-apr.component.css']
})
export class TicketAprComponent implements OnInit {
  ...
  displayedColumns: string[] = ['idTicket', 'titre','type','priorite','statue', 'dateHeureCreation', 'lieu','description','idDetailM','idDemandeur','action'];
  ...
  getAllTickets(){
    ...
    this.TUS.getTicket().subscribe({
      next:(res)=>{
        ...
        for(let item of res){
       if ( item.statue=="Pour Aprobation" ){
